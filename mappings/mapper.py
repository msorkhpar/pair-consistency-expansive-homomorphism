...
class MapGtoH:
    ...
    def _uv_to_ij_cost(self, u, v, uv_length, i, j, ij_length) -> MappingCost:
        ...
        length = length_cost(
            self.gamma, self.g.total_length, self.h.total_length, self.h.min_length, uv_length, ij_length, u, v, i, j
        )
    ...
    def calculate_mapping_costs(self) -> dict[Mapping, MappingCost]:
        ...
        for uv in self.g.edges(directed=True):
            ...
            for ij in self.h.paths():
                costs[Mapping(uv, ij)] = self._uv_to_ij_cost(uv.v1, uv.v2, uv_length, ij.v1, ij.v2, ij.length)
        return costs
